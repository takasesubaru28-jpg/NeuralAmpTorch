...
namespace Steinberg {
    namespace Vst {
        ...
        {
            // process() accesses channelBuffers32, so advertising 64-bit
            // support would make hosts pass buffers with the wrong layout.
        ...
        tresult PLUGIN_API MyVSTProcessor::setupProcessing(ProcessSetup& setup)
        {
            if (!running) {
                try {
                    ...
                    if (!loadBundledLibTorchRuntime(runtimeError)) {
                        OutputDebugStringA(runtimeError.c_str());
                        return kResultFalse;
                    }
                    at::set_num_threads(1);
                    inputTensorRef = torch::zeros(
                        {1, static_cast<int64_t>(input_size), 1}, torch::kFloat32);
                    paramTensorRef = torch::zeros({1, param_dim}, torch::kFloat32);
                    model = torch::jit::load(folder + "\\LSTM_1024.pt");
                    model.eval();
                    modelLoaded = true;
                    running = true;
